...
import { getAiMenuImageQualityWarnings, normalizeAiMenu } from "../../platform/ai-menu";
...
export default async (request: Request) => {
  ...
  try {
    const payload = await request.json() as { imageBase64?: string; mimeType?: string; storeId?: string; width?: number; height?: number; originalWidth?: number; originalHeight?: number };
...
5. 相同品項不要重複。若照片模糊、裁切、反光、解析度偏低，或看起來像截圖、外送平台畫面、社群轉貼圖，請在 warnings 明確提醒店家。
6. 每個品項尋找照片中「只呈現該餐點」的照片區域，imageBox 使用 [ymin,xmin,ymax,xmax]，座標依整張輸入圖正規化為 0–1000 的整數。框完整餐點主體並保留一點安全邊界；不要包含品名、價格或其他文字；若沒有專屬或可明確對應的照片，imageBox 輸出空陣列 []。
7. 只輸出 JSON 物件：{"items":[{"name":"","price":0,"category":"","description":"","badge":"","imageBox":[120,80,360,420],"optionGroups":[{"name":"","required":false,"min":0,"max":1,"options":[{"name":"","price":0}]}]}],"warnings":[]}。所有欄位都必須存在。` }],
    ...
    const qualityWarnings = getAiMenuImageQualityWarnings({ mimeType: payload.mimeType, width: payload.originalWidth || payload.width, height: payload.originalHeight || payload.height });
    return json({ products: normalized.products, warnings: Array.from(new Set([...qualityWarnings, ...normalized.warnings])), model: MODEL });
  } catch (error) {
